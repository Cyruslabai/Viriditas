...
`-- viriditas/
    ...
    |-- training/           Training package (implemented; notebook is a thin wrapper)
    |-- inference/          Inference package (implemented for plant identification)
    |-- evaluation/         Evaluation package (implemented; analysis notebook cleanup still pending)
    |-- models/             Model definitions / artifacts handling
    `-- utils/              Shared utilities
```

`src/agriai/` is no longer present in the active source tree. `src/viriditas/`
is the authoritative package namespace.
...
- `src/viriditas/training/` - plant identification training package,
  implemented. The trainer reads artifact-backed metadata on Kaggle when
  available and uses the saved plant label map for class-index alignment.
- `src/viriditas/inference/` - plant identification inference package,
  implemented with lazy cached model and label-map loading.
- `src/viriditas/evaluation/` - reusable evaluation package, implemented;
  the analysis notebook still needs its known cleanup pass.
...
- `notebooks/02_train_plant_model.py` — plant identification training,
  thin wrapper over `src/viriditas/training/`.
...
### Decision: Use artifact-backed metadata and saved label maps for Kaggle training
Date: 2026-08-06

Reason: Kaggle working directories are temporary, while generated metadata and
label maps must remain stable across training and inference reruns. Rebuilding
class indices from a dataframe risks drifting away from the label map used by
saved artifacts.

Action: extended path configuration to discover an attached
`viriditas-artifacts` dataset and common metadata layouts; updated plant
training to read artifact-backed `plant_id_dataset.csv` when available and to
load `label_map_plants.json` through the shared inference loader.

Benefits: repeatable Kaggle training sessions; stable class-index alignment;
clear separation between read-only artifact inputs and writable model outputs.
...
- Implemented training, inference, and evaluation packages under
  `src/viriditas/`; notebooks are now thin wrappers or pending cleanup.
- Added artifact-backed metadata and saved label-map loading for Kaggle plant
  training reruns.
- Implemented lazy cached plant model and label-map loading for inference.
- Removed/reconciled the legacy `src/agriai/` blocker; `src/viriditas/` is the
  active package namespace.
...
- Cleaning and validating the plant analysis workflow before treating its
  reports as reliable.
- Converting remaining notebook placeholders into real thin wrappers or
  removing them.
...
1. Fix known issues in `notebooks/03_model_analysis.py` (local metadata path,
...
thin. Implemented for the data pipeline, training, inference, and evaluation;
analysis validation and remaining notebook cleanup are still pending.

## Next Tasks

1. Fix and rerun `notebooks/03_model_analysis.py`; save summary metrics.
2. Decide on the 7 unlabeled apple images.
3. Populate or remove empty notebook placeholders.
4. Create `03_train_disease_model.ipynb` and train the disease classification
   model.
5. Begin FastAPI backend design around the plant inference package.
6. Begin Flutter client planning.

## Future Roadmap

Phase 1 — Configuration: done
Phase 2 — Centralized Preprocessing: done
Phase 3 - Training Package: done
Phase 4 - Inference Package: plant identification implemented
Phase 5 - Evaluation Package: implemented; analysis validation pending
