...
- Added Kaggle artifact-aware metadata resolution for training reruns: `PlantIdentifierTrainer` now requests artifact-backed `plant_id_dataset.csv` when available.
- Updated training to load the saved `label_map_plants.json` instead of rebuilding class indices from the current dataframe, keeping model output classes aligned with generated metadata.
- Extended path configuration to discover a `viriditas-artifacts` Kaggle dataset and common nested metadata layouts.
- Updated inference resource loading to lazily cache the canonical best plant model and plant label map through environment-aware path resolution.
- Added trainer/debug logging for artifact metadata paths, dataset row counts, class counts, class names, split sizes, and model output shape.

### Documentation

- Updated README, architecture notes, development guide, Kaggle runbook, TODOs, project plan, and journal to reflect artifact-backed metadata, saved label-map loading, lazy inference loading, and the cleared legacy `src/agriai/` blocker.
